...
export function mostrarPantallaLoginRegistro(contenedor) {
  ...
  btnAccion.addEventListener('click', async () => {
    ...
    try {
      if (modoRegistro) {
        if (!correo || !contrasena || !tipoUsuario || !nombreCompleto) throw new Error('Complete todos los campos.');
        if (!aceptaTerminos) throw new Error('Debe aceptar los Términos y Condiciones.');
        ...
        let rol, estado;
        if (tipoUsuario === 'cliente') { rol = 'cliente'; estado = 'aprobado'; }
        else { rol = tipoUsuario; estado = 'pendiente'; }

        await setDoc(doc(db, 'usuarios', uid), {
          correo, nombreCompleto, rol, estado, fechaRegistro: new Date()
        });
        ...
        if (!correo || !contrasena) throw new Error('Ingrese correo y contraseña.');
        const credenciales = await signInWithEmailAndPassword(auth, correo, contrasena);
        const uid = credenciales.user.uid;
        const datos = await getDoc(doc(db, 'usuarios', uid));
        if (!datos.exists()) throw new Error('Usuario no encontrado.');
        const usuario = datos.data();
        if (usuario.estado === 'pendiente') { await signOut(auth); throw new Error('⏳ Su cuenta aún no ha sido aprobada. Espere.'); }
        if (usuario.estado === 'rechazado' || usuario.estado === 'suspendido') { await signOut(auth); throw new Error('❌ No puede ingresar. Consulte con el administrador.'); }
      }
    } catch (error) { mensajeError.textContent = '❌ ' + error.message; }
  });
}
